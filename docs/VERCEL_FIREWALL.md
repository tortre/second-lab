...
Vercel currently allows one rate-limit rule per Hobby project and 40 on Pro. Because Second Lab needs separate access, review, and coaching rules, the full three-rule design requires Pro or higher. Confirm the production plan before treating this control as active.

## Required production rules

All rules use a 600-second window, count by client IP, and return HTTP 429 after the limit.

### 1. Ten judge-code attempts per IP per 10 minutes

Match request path `/api/access` and method `POST`. Rate limit to 10 requests per 10 minutes per IP and return 429.

Vercel's natural-language rule builder prompt:

> Rate limit POST requests to /api/access to 10 per 10 minutes per IP, then return 429.

### 2. Three live review requests per IP per 10 minutes
...
### 3. Twenty coaching requests per IP per 10 minutes
...
5. From a test IP, verify the eleventh access attempt, fourth multipart review, and twenty-first coaching request return 429. Separately repeat the cached demo more than three times and confirm it remains usable.

## Deployment evidence

- [ ] Access rule appears active in the production project.
